...
const Dashboard = () => {
  ...
  const fetchUploads = async (id) => {
    try {
      const response = await axios.get(`http://:5000/api/uploads?user_id=${id}`);
  ...
  const handleUpload = async () => {
    ...
    setIsUploading(true);
    try {
      await axios.post('http://:5000/api/upload', formData, {
        headers: {
  ...
  const handleAnalyze = async (uploadId) => {
    ...
    try {
      await axios.post(`http://:5000/api/analyze/${uploadId}`);
      fetchUploads(userId);
      toast({
            ...
            <Box key={upload.id} borderWidth="1px" borderRadius="lg" p={4} mt={4}>
              ...
              <Flex direction={["column", "row"]} justify="space-between">
                <Box flex="1">
                  ...
                  <Image 
                    src={`http://:5000/uploads/${upload.filename}`} 
                    alt={upload.filename}
                    maxH="300px"
                    objectFit="contain"
                  />
                ...
                {upload.analyses && upload.analyses.length > 0 && (
                  <Box flex="1" ml={[0, 4]} mt={[4, 0]}>
                    ...
                    <Image 
                      src={`http://:5000/output/${upload.analyses[0].result_path}`}
                      alt="Analyzed Image"
                      maxH="300px"
                      objectFit="contain"
                    />
