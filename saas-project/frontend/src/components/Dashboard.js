...
const Dashboard = () => {
  ...
  const fetchUploads = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/api/uploads?user_id=${id}`);
  ...
  const handleAnalyze = async (uploadId) => {
    ...
    try {
      await axios.post(`http://localhost:5000/api/analyze/${uploadId}`);
      fetchUploads(userId);
      toast({
              ...
              <Flex direction={["column", "row"]} justify="space-between">
                ...
                {upload.analyses && upload.analyses.length > 0 && (
                  <Box flex="1" ml={[0, 4]} mt={[4, 0]}>
                    ...
                    <Image 
                      src={`http://localhost:5000/output/${upload.analyses[0].result_path}`}
                      alt="Analyzed Image"
                      maxH="300px"
                      objectFit="contain"
                    />
